...
Todo componente visual reutilizável mora em `src/design-system/` (catálogo em `DESIGN_SYSTEM.md`) e é consumido por todos os módulos de feature — nenhuma tela reimplementa botão, badge, card ou campo de formulário. O documento do próprio projeto já registra o "antes": cada tela reimplementava as mesmas classes Tailwind com pequenas variações de altura/cor/borda. É o motivo pelo qual as 15 interfaces do item 2 têm alta variação de *conteúdo* e *fluxo*, mas identidade visual consistente — reuso sem engessar a personalização.
